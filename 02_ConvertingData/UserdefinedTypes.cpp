...
int main() {
    negative_number n1 = boost::lexical_cast<negative_number>("-100");
    assert(n1.value_without_sign() == 100);

    int i = boost::lexical_cast<int>(n1);
    assert(i == -100);

    typedef boost::array<char, 10> arr_t;
    arr_t arr = boost::lexical_cast<arr_t>(n1);
    assert(arr[0] == '-');
    assert(arr[1] == '1');
    assert(arr[2] == '0');
    assert(arr[3] == '0');
    ...
    // wide chars
    negative_number n2 = boost::lexical_cast<negative_number>(L"-1");
    assert(n2.value_without_sign() == 1);
    typedef boost::array<wchar_t, 10> warr_t;
    warr_t arr2 = boost::lexical_cast<warr_t>(n2);
    assert(arr2[0] == '-');
    assert(arr2[1] == '1');
    assert(arr2[2] == '\0');
}
